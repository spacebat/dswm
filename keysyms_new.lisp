;;; -*- Mode: LISP; Syntax: Common-lisp; Package: dswm.modules.web -*-
...
(defvar *key-name-syms* nil
  "Defines list of keynames and related keysyms to it")

(defstruct dswm-key
  "defines keyname->keysyms"
  name syms)

(defun get-dswm-key-by-name (name dswm-key-names)
  "Gets dswm key structure by key name"
  (cond ((null (car dswm-key-names)) nil)
	((not (equal name (dswm-key-name (car dswm-key-names))))
	 (get-dswm-key-by-name name (cdr dswm-key-names)))
	(t (car dswm-key-names))))

(defun get-dswm-key-by-keysym (keysym dswm-key-names)
  "Gets dswm key structure by keysym"
  (cond ((null (car dswm-key-names)) nil)
	((not (member keysym (dswm-key-syms (car dswm-key-names))))
	 (get-dswm-key-by-keysym keysym (cdr dswm-key-names)))
	(t (car dswm-key-names))))

(defun define-keysym (keysym name)
  "Define a mapping from a keysym name to a keysym."
  (let ((key-by-name (get-dswm-key-by-name name *key-name-syms*))
	(key-by-sym (get-dswm-key-by-keysym keysym *key-name-syms*))
	(new-key (make-dswm-key :name name :syms (list keysym))))
    (cond
      ((not (null key-by-name))
       (remove-from-list *key-name-syms* key-by-name)
       (add-to-list *key-name-syms* new-key))
      ((not (null key-by-sym))
       (remove-from-list *key-name-syms* key-by-sym)
       (add-to-list *key-name-syms*
       		    (make-dswm-key
       		     :name (dswm-key-name key-by-sym)
       		     :syms (remove-from-list (dswm-key-syms key-by-sym) keysym)))
       (add-to-list *key-name-syms* new-key))
      (t
       (add-to-list *key-name-syms* new-key)))))
...
(defun keysym-name->keysym (name)
  ...
  (let ((sym (get-dswm-key-by-name name *key-name-syms*))
	(x 'nil))
    (if (not (null sym))
	(car (dswm-key-syms sym))
	x)))

(defun keysym->keysym-name (keysym)
  "Return the name corresponding to KEYSYM."
  (declare (type (unsigned-byte 32) x))
  (let ((name (get-dswm-key-by-keysym keysym *key-name-syms*))
	(x 'nil))
    (if (not (null name))
	(dswm-key-name name)
	x)))
...
(define-keysym #x06bd "backslash") ;U+0490 CYRILLIC CAPITAL LETTER GHE WITH UPTURN
...
;; (define-keysym #x06cb "r")  ;U+043A CYRILLIC SMALL LETTER KA
;; (define-keysym #x06cc "k")  ;U+043B CYRILLIC SMALL LETTER EL
;; (define-keysym #x06cd "v")  ;U+043C CYRILLIC SMALL LETTER EM
;; (define-keysym #x06ce "y")  ;U+043D CYRILLIC SMALL LETTER EN
;; (define-keysym #x06cf "j")    ;U+043E CYRILLIC SMALL LETTER O
;; (define-keysym #x06d0 "g")  ;U+043F CYRILLIC SMALL LETTER PE
;; (define-keysym #x06d1 "z")  ;U+044F CYRILLIC SMALL LETTER YA
;; (define-keysym #x06d2 "h")  ;U+0440 CYRILLIC SMALL LETTER ER
;; (define-keysym #x06d3 "c")  ;U+0441 CYRILLIC SMALL LETTER ES
;; (define-keysym #x06d4 "n")  ;U+0442 CYRILLIC SMALL LETTER TE
;; (define-keysym #x06d5 "e")    ;U+0443 CYRILLIC SMALL LETTER U
;; (define-keysym #x06d6 ";") ;U+0436 CYRILLIC SMALL LETTER ZHE
