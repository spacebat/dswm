...
(defmacro define-ds-event-handler (event keys &body body)
  (let ((fn-name (gensym))
        (event-slots (gensym)))
    `(labels ((,fn-name (&rest ,event-slots &key ,@keys &allow-other-keys)
      ...
      (setf (gethash ,event *event-fn-table*) #',fn-name))))
                                        ...
                                        ;(define-ds-event-handler :map-notify (event-window window override-redirect-p)
                                        ;  )
...
(defun handle-unmanaged-window (xwin x y width height border-width value-mask)
  "Call this function for windows that dswm isn't
...
(define-ds-event-handler :configure-request (stack-mode #|parent|# window #|above-sibling|# x y width height border-width value-mask)
...
(define-ds-event-handler :configure-notify (stack-mode #|parent|# window #|above-sibling|# x y width height border-width value-mask)
...
(define-ds-event-handler :map-request (parent send-event-p window)
  (unless send-event-p
...
(define-ds-event-handler :unmap-notify (send-event-p event-window window #|configure-p|#)
...
;;(define-ds-event-handler :create-notify (#|window parent x y width height border-width|# override-redirect-p))
...
(define-ds-event-handler :destroy-notify (send-event-p event-window window)
...
(define-ds-event-handler :key-press (code state #|window|#)
...
(defun handle-dswm-commands (root)
  "Handle a DSWM style command request."
  (let* ((win root)
         (screen (find-screen root))
         (data (xlib:get-property win :dswm_command :delete-p t))
         (cmd (bytes-to-string data)))
    (let ((msgs (screen-last-msg screen))
          (hlts (screen-last-msg-highlights screen))
          (*executing-dswm-command* t))
      (setf (screen-last-msg screen) '()
            (screen-last-msg-highlights screen) '())
      (eval-command cmd)
      (xlib:change-property win :dswm_command_result
...
(define-ds-event-handler :property-notify (window atom state)
  ...
  (case atom
    ...
    (:dswm_command
     ...
     (let* ((screen (find-screen window)))
       (when (and (eq state :new-value)
                  screen)
         (handle-dswm-commands window))))
    (t
     (let ((window (find-window window)))
       (when window
         (update-window-properties window atom))))))

(define-ds-event-handler :mapping-notify (request start count)
...
(define-ds-event-handler :selection-request (requestor property selection target time)
  (send-selection requestor property selection target time))

(define-ds-event-handler :selection-clear ()
...
(define-ds-event-handler :exposure (window x y width height count)
  (let (screen ml)
    (when (zerop count)
      (cond
...
(define-ds-event-handler :reparent-notify (window parent)
  (let ((win (find-window window)))
    (when (and win
...
(define-ds-event-handler :client-message (window type #|format|# data)
...
(define-ds-event-handler :focus-out (window mode kind)
...
(define-ds-event-handler :enter-notify (window mode)
...
(define-ds-event-handler :button-press (window code x y child time)
...
(defun handle-event (&rest event-slots &key display event-key &allow-other-keys)
  ...
  (let ((eventfn (gethash event-key *event-fn-table*))
    ...
    (when eventfn
      ...
      ;; in dswm. So far the only slot that seems to be affected is
      ...
      (handler-case
          (progn
            ;; This is not the dswm top level, but if the restart
            ...
            (with-simple-restart (top-level "Return to dswm's top level")
